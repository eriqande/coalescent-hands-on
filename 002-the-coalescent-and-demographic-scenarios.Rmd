...
```{r}
system2(command = ms_binary(), args = "10 6 -T -G .055", stdout = "out.tree", stderr = FALSE)
ctree <- read.tree("out.tree")
names(ctree) <- 1:6
ggtree(ctree, layout = "rectangular") +
  facet_wrap(~.id, ncol  = 3) +
  geom_tiplab(size = 2)
```
Yep, somewhat longer terminal branches. Play around with different settings and check out the
results.

### Instantaneous population size changes

Past changes in the populations are controlled by values `-eSomething` options.  Where
the e stands for event.  The simplest of these are instantaneous changes in population
size.  The simplest is:

* `-en t i x` which means "Set subpop $i$ size to $x times N0$ at time $t$ and growth rate to zero.
...
you can read the full documentation: [msdoc.pdf](./msdoc.pdf).  But for now, we will
...
backward in time.  But, _forward in time_ this corresponds to poupulation _splitting_.\

* `-es t i p` which means split subpopulatin $i$ into two subpopulations $i$ and $npop + 1$ $t$
...
3. Hand in:  Write down verbal model of the population demography, and then
submit your R code and the figure of trees you made.
